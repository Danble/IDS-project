...
function copyGlossToTSV(sheet_info: GlossesSheetData, gloss_data: GlossData): void {
  ...
  const ids_gloss_column = idsDataSheet.getRange(`${idsGlossColumn}2:${idsGlossColumn}`).getValues();
  ...
  chapter_id_values.forEach((row, i) => {
    ...
    if (match_index !== -1) {
      first_empty_column_range.getCell(i + 1, 1).setValue(ids_gloss_column[match_index][0]);
    }
  });
...
function copySemanticDomainsToTSV(sheet_info: SemanticDomainsSheetData): void {
  ...
  chapter_id_values.forEach((row, i) => {
    // const match_index = row.findIndex((value) => value[0] === )
